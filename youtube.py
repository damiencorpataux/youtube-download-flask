...
def videos(id, resolution=None, extension=None):
    yt = _yt(id)
    return {
        'id': yt.video_id,
        'title': yt.title,
        ...
        'videos': sorted([vars(v) for v in yt.filter(extension, resolution)],
                         reverse=True,
                         key=lambda x: (x.get('resolution'),
                                        x.get('extension')))
    }

def streamurl(id, resolution=None, extension=None):
    """
    Returns the best possible video stream url
    according the given parameters.
    """
    try:
        video = videos(id, resolution, extension)['videos'][0]
    except IndexError:
